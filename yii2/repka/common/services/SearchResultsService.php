...
class SearchResultsService
{
    ...
    public function create(Project $project): SearchResult
    {
        ...
        if ($searchResult->save()) {
            $this->queue->push(
                new UpdateJob(['resultId' => $searchResult->id, 'cron' => $this->cron])
            );
        }
    ...
    public function findSearchResults(SearchResultFilter $filter, ?int $guestLinkId = null): array
    {
        ...
        if ($guestLinkId === null) {
            foreach ($filter->getProject()->projectMarkerDomains as $markerDomain) {
                $domainHash                  = md5($markerDomain->domain);
     ...
     * @throws \Exception
     */
    public function actualizeData(SearchResult $searchResult, $cron): void
    {
        if (!empty($searchResult)) {
            ...
            try {
                ...
                if ($cron) {
                    ...
                    $this->checkNewDataAndSendEmail($searchResult, $list, $requests, $regions, $searchEngines);
                }
                /*******************************/
     ...
     */
    public function checkNewDataAndSendEmail($searchResult, $searchResultList, $requests, $regions, $searchEngines)
    {
        try {
