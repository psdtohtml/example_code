...
class BalanceController extends \backend\controllers\SiteController
{
    ...
    public function actionChangeBalance($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if($this->changeBalance($user, abs($model->balance), $model->note, $model->company)) {
                $params = [
                    'balance' => abs($model->balance),
                    'company' => abs($model->company),
                    'note' => abs($model->note),
                ];
                $user->sendMail(6, $params);
                Yii::$app->session->setFlash('success', 'Баланс пользователя успешно изменен');
